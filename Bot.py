...
from telebot.types import InlineKeyboardMarkup, InlineKeyboardButton, SwitchInlineQueryChosenChat, InlineQueryResultsButton, ForceReply
...
def send_welcome(message):
	...
	if payload is not None:
		...
		bot.send_message(message.chat.id, f"Question: {question}\nPlease reply with your answer to the question to unlock the answer!", reply_markup=ForceReply(input_field_placeholder='Your Answer:'))
		bot.set_state(message.from_user.id, f"unlock_answer|{query_id}|{user_id}|{message.from_user.username}")
	else:
		bot.send_message(message.chat.id, "Please Select:", reply_markup=gen_main_menu())
...
def gen_back_button(state):
	markup = InlineKeyboardMarkup()
	markup.row_width = 1
	markup.add(InlineKeyboardButton("Back", callback_data=f"cb_{state}", ))
	return markup
...
def callback_query(call):
	if call.data == "cb_main_menu":
		bot.edit_message_text( "Please select:", reply_markup=gen_main_menu(), chat_id=call.message.chat.id ,message_id=call.message.id)
	elif call.data == "cb_contact":
		bot.edit_message_text( "Please contact me at: @junkeai", reply_markup=gen_back_button("main_menu"), chat_id=call.message.chat.id ,message_id=call.message.id)
	elif call.data == "cb_question_deck":
		bot.edit_message_text("Please select a question deck:", reply_markup=gen_question_deck_menu(), chat_id=call.message.chat.id ,message_id=call.message.id)
	elif call.data == "cb_custom_question":
		bot.edit_message_text("Please type and send your custom question", reply_markup=gen_back_button("main_menu"), chat_id=call.message.chat.id ,message_id=call.message.id)
		...
		bot.edit_message_text(f"Please select a question from the {deck} deck:\n\nQuestion: {question}", reply_markup=gen_questions(0, deck), chat_id=call.message.chat.id ,message_id=call.message.id)
		...
		bot.edit_message_text(f"Please select a question\n\nQuestion: {question}", reply_markup=gen_questions(idx, deck), chat_id=call.message.chat.id ,message_id=call.message.id)
		bot.set_state(call.from_user.id, f"deck_answer|{question}|")
	elif call.data.startswith("cb_question"):
		idx = int(call.data.split("_")[2])
		deck = call.data.split("_")[3]
		question = decks[deck][idx]
		bot.send_message(call.message.chat.id, f"Question: {question}\n\nPlease Answer!", reply_markup=ForceReply(input_field_placeholder='Your Answer:'))
		bot.set_state(call.from_user.id, f"deck_answer|{question}|")
		...
		if len(data["replies"])>=requirement:
			...
			bot.edit_message_text(f"Question: {data['question']}\n\n@{sender_id}'s answer: Locked 🔐\n{requirement-count} more answer(s) needed to unlock!\n\nTalk to @bumble_tele_bot to ask more questions!", inline_message_id=call.inline_message_id, reply_markup=gen_reveal_answer(query_id, sender_id))
		pass
def send_private_chat(question_answer_requirement):
	markup = InlineKeyboardMarkup()
	markup.row_width = 1
	markup.add(InlineKeyboardButton("Select chat to send question", switch_inline_query=question_answer_requirement))
	return markup
...
def handle_message(message):
	...
	if status == "custom_answer":
		...
		answer = message.text
		bot.send_message(message.chat.id, f"Please state the number of required answers to unlock your answer!")
		...
		bot.send_message(message.chat.id, f"Question: {message.text}\n\nPlease reply with your answer to the question!", reply_markup=ForceReply(input_field_placeholder='Your Answer:'))
		bot.set_state(message.from_user.id, f"custom_answer|{message.text}|")
	elif status == "deck_answer":
		question = state.split('|')[1]
		answer = message.text
		bot.send_message(message.chat.id, f"Please state the number of answers required to unlock the answer!")
		bot.set_state(message.from_user.id, f"lock_requirement|{question}|{answer}")
		...
		bot.send_message(message.chat.id, f"Answer submitted! Go back to the chat and click 'Reveal Answer' to check @{user_id}'s answer!")
...
def gen_reveal_answer(query_id, sender_id):
	markup = InlineKeyboardMarkup()
	markup.row_width = 1
	markup.add(InlineKeyboardButton("Reveal Answer", callback_data=f"cb_reveal_answer_{query_id}_{sender_id}"))
	markup.add(InlineKeyboardButton("Answer Question", switch_inline_query_current_chat=f"answer_question_{query_id}_{sender_id}"))
	return markup
...
def answer_inline_query(inline_query):
	try:
		...
		bot.answer_inline_query(inline_query.id, [], switch_pm_text="CLICK ME to answer the qn!", switch_pm_parameter=f"{query_id}_{sender_id}")
...
def default_query(inline_query):
	try:
		...
		print(f"question: {question} answer: {answer} inline_query_id: {inline_query.id} from: {inline_query.from_user.username}")
		if question == "":
			question = "random_question()"
		r = types.InlineQueryResultArticle(inline_query.id, f"CLICK ME to send!", 
		types.InputTextMessageContent(f"\nQuestion: {question}\n\n@{inline_query.from_user.username}'s answer: Locked 🔐\n{requirement} more answer(s) needed to unlock!\n\nTalk to @bumble_tele_bot to ask more questions!"), 
		description="",
		 ...
		 thumbnail_width=70, thumbnail_height=70, reply_markup=gen_reveal_answer(inline_query.id, inline_query.from_user.username))
...
def test_chosen(chosen_inline_result):
	...
	temp = {chosen_inline_result.result_id:{
